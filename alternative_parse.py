...
class Parser:
    ...
    def parse_sentence(self,sentence,chunkerator):
        ...
        retagged = chunkerator.initial_tag(sentence)
        print retagged
        verb_relats = parse_by_verb(retagged)
        relations.extend(verb_relats)
        print relations
        return relations
...
senttt2 = 'The man didn\'t already make some green engines, and The woman did yoga and the girl isn\'t ever upset'
text1 = "Today was a good day.  Yesterday was a bad day."
#c = Chunkerator(rules, True)
#sent3 =  c.chunk_sent(senttt2)
#sent4= c.initial_tag(sent3)



def find_internal_args(index,sentence,currVerb,currArgs):
    yy = 1
    scanRight = True
    while index + yy < len(sentence) and scanRight:
        if sentence[index+yy][1][0] == "N":
            currArgs.append(sentence[index+yy][0])
            yy += 1
        elif sentence[index+yy][1][0:2] == "RB":
            currVerb.insert(0,sentence[index+yy][0])
            yy += 1
        else:
            scanRight = False

def find_external_arg(index,sentence,currVerb,currArgs):
    zz = 1
    scanLeft = True
    while index - zz >= 0 and scanLeft:
        if sentence[index-zz][1][0] == "N":
            currArgs.append(sentence[index-zz][0])
            zz += 1
        elif sentence[index-zz][1][0:2] == "RB":
            currVerb.insert(0,sentence[index-zz][0]) 
            zz += 1
        else:
            scanLeft = False

def find_negs(currVerb):
    currPolarity = True
    neg_words = ['not','never', 'isn\'t','was\'nt','hasn\'t','haven\'t','didn\'t','don\'t','doesn\'t']
    for x in currVerb:
        if x in neg_words:
...
def stringify_currVerb(currVerb):
    g = ""
    for x in currVerb:
        g += str(x)+" " 
    g = g.rstrip()
...
def parse_by_verb(sentence):
    
    relats_to_make = []
    
    for xx in xrange(len(sentence)):
        currArgs = []
        currVerb = []
        if sentence[xx][1][0:2] == "VB":
            currVerb.append(sentence[xx][0])
        if currVerb != []:
            ##find external argument/subject of current verb if any
            find_external_arg(xx,sentence,currVerb,currArgs)
            ##find internal arguments of current verb if any
            find_internal_args(xx,sentence, currVerb,currArgs)
            currPolarity = find_negs(currVerb)
            #stringify currVerb
            g = stringify_currVerb(currVerb)            
            relats_to_make.append((g, currArgs, currPolarity))
    return relats_to_make    


#xxx = senttt2.split()
#c = Chunkerator(rules, True)
#sent3 =  c.chunk_sent(xxx)
#
#sent4= c.initial_tag(sent3)
#print "XXXXX"
#print sent4
